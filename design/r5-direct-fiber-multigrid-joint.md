...
坐标修正后的原始配置在 calibration seed 1801 上已经通过三个网格的结构、collocation 收缩、
轨迹收缩和在线不退化门，因此 C1--C3 容量放宽不再执行。正式三 seed 保持原始
$\rho=0.35$、25% 增益信赖域和误差尺度 1，只按上述规则切换到全新 validation seed 1851。

第一次 fresh validation（seed 1851）显示三个模型 seed 都只在 $n=63$ 的 collocation 最坏门
失败，余量为 $-0.0555$、$-0.0473$、$-0.0256$；其余网格 collocation、三个网格全部轨迹、
在线误差和结构门均通过。固定 $T_0$ 的对应余量约为 $-0.18$，说明 $T_\phi$ 已显著缩小缺口，
但不能把负余量称为证书成功。locked test 仍未生成。

因此把 seed 1851 降格为第二个 calibration split，并用模型 seed 1301 做以下归因筛选：

| 方案 | $\rho$ | 增益信赖域 | 回答的问题 |
|---|---:|---:|---|
| R-T | 0.65 | 0.25 | 只增加 $T_\phi$ 容量是否足够 |
| R-B | 0.35 | 0.50 | 只增加 $B$ 容量是否足够 |
| R-BT | 0.65 | 0.50 | 两者是否需要共同增加 |

三项保持误差尺度 1、80 epoch、原训练 split 和同一 calibration 数据。候选必须通过结构与
在线门，并使三个网格中最坏 collocation 余量至少达到 $+0.02$，才允许进入下一次正式运行；
在合格候选中先最大化最坏 collocation 余量，再最大化最坏轨迹余量，并优先选择较小预算。
下一次正式 validation 使用从未读取的 seed 1871，locked test seed 1901 保持不变。
